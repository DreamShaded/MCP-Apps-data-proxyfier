...
### Режим данных: кэш или вживую

Каждый результат в UI помечен бейджем источника — видно, что отработало:

- **из кэша ⚡** — отдано из прогретого кэша мгновенно (`source: hit`),
- **вживую 🌐** — получено живым запросом через браузер (`source: miss`),
- **нет связи ⚠** — живой источник не ответил, показан запасной вид (`fallback`).

Рядом — время записи («5 мин назад») из `fetchedAt`.

Режим переключается двумя способами:

- **На один запрос** — аргумент `fresh: true` у read-инструмента обходит кэш и идёт
  вживую («покажи живой запрос», «самые свежие данные»). Управляется прямо
  формулировкой запроса к модели, перезапуск не нужен.
- **Глобально на старте** — `MCP_CACHE_MODE`:
  - `auto` (по умолчанию) — хит → мгновенно, мисс → вживую;
  - `cache` — только кэш, в браузер не ходим (зал без сети / гарантированная
    скорость демо); промах отдаёт запасной вид, а не живой запрос;
  - `live` — всегда вживую, кэш на чтение игнорируется.

  Per-call `fresh: true` переопределяет глобальный дефолт (например, разово
  показать живой запрос в зале, запертом на `cache`).
